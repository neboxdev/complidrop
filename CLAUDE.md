...
- **Frontend testid policy**: prefer accessible-text selectors (`getByText` / `getByRole` / `getByLabelText`) in component and E2E tests. Reach for `data-testid` only when text selectors are ambiguous-by-design — e.g. a status badge whose label collides with section copy or sibling badges. Place the testid on the element whose state is asserted (the badge / input / row), NOT on a wrapper container. See `extraction-status` / `compliance-status` on [documents/[id]/page.tsx](frontend/src/app/(dashboard)/documents/[id]/page.tsx) ([#92](https://github.com/neboxdev/complidrop/issues/92)) for the canonical shape.
