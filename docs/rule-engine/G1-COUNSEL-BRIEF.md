...
| **CLM-1** | Additional-insured copy: reword "Names you as additional insured" → "Certificate indicates…"; the certificate cannot prove AI status (endorsement needed). **UI/API copy now STAGED** behind `ComplianceClaims:CorrectedAdditionalInsuredWording` (default OFF, copy-only, ADR 0042); marketing-site copy still to do at flip. | A | Marketing + UI (#396) | ⬜ | §C, TRR §3/§7 |
...
- Additional-insured wording (CLM-1) → set
  ...
  marketing-site copy has no flag and is edited at the same time (§C, ADR 0042).
