...
| **CLM-7** | Settings promised "Permanently deletes your account and organization data. This can't be undone." (retired — quoted plain, not as copy to bless) while `AuthEndpoints.DeleteAccount` scrubs only the account holder's email + name and tombstones the rest, and the Privacy Policy promised deletion "within a reasonable period" with no purge job anywhere behind it. **The corrected copy now SHIPS** (#398, ADR 0013 Amendment 1), on by default and behind no flag, on ADR 0047's reasoning rather than ADR 0043's: copy that overstates deletion is a defect either way round, so a default-OFF flag would ship the fix and leave the deception live. **Behaviour is deliberately UNCHANGED** — nothing new is purged, no schedule was invented, and the retained set is still ADR 0013's. **Copy to bless — (a)–(d) below, all shipped, none reaching a verdict** (§C carries the same items with the reasoning; a new item is added in BOTH places or in neither): **(a)** the Settings closure notice read by the customer at the moment of decision, *"Closing signs you out for good and clears your name and email from your account record."* and *"Your vendors, documents, the files uploaded for them, and our record of account activity are kept, and we handle them as described in our Privacy Policy."*; **(b)** the Privacy Policy's replacement retention sentences, *"Closing makes the account unusable — but on its own it is not a deletion of everything the account held."* and *"We have not set a fixed disposal period for those records, so they stay with us until you ask us to delete them."*; **(c)** the per-item removal notice, *"This removes the document from your records. You won't be able to undo it."* — scoped to the customer because both endpoints soft-delete and `DeleteDocument` retains the blob on purpose; **(d)** the data-export description, *"The uploaded files themselves aren't included."* **THE OPEN DECISION, which #398 deliberately did not answer in code: should CompliDrop build a post-closure hard purge (blobs + vendor contact PII) and publish a retention schedule, or is retention-with-deletion-on-request the posture?** Three rulings beyond the wording: **(i)** is a stated **retention schedule** required of us at all, or is "no fixed disposal period, deleted on request" a lawful and honest answer for a US-only B2B SaaS — noting that publishing a period no job enforces is the very defect #398 reports, in a new sentence; **(ii)** does closure-plus-scrub satisfy a **CCPA §1798.105 delete request** from the account holder, given that the vendor contact details, document files and audit rows behind their org survive it, or must the request reach those too; **(iii)** the reader we cannot ask — a **vendor named on a certificate, or the account-less uploader of one**, whose PII we retain under a customer's closed account with no relationship to us and no notice from us; the policy routes them to the business and offers our contact details, and §C asks whether that is enough. | A | Settings + privacy (#398) | ⬜ | §C |

**Launch-blocking:** RE-1..RE-5 (rule engine), TPL-A1..A4 + TPL-B1..B3 (templates &
dollar minimums), CLM-1..CLM-7 (copy/privacy). *Refine-later:* TPL-B4..B8.
...
The deletion copy (CLM-7) is that shape again and for the sharpest version of the
reason: a claim that overstates deletion is a defect whichever way the flag sits, so
#398 shipped the truthful wording immediately and left the *behaviour* — what, if
anything, is purged after closure — entirely to this gate. Sign-off is a wording
confirmation plus one decision (purge-and-publish-a-schedule vs
retention-with-deletion-on-request); building the purge would need its own ADR,
because ADR 0013 records soft-delete-plus-scrub as a deliberate choice.
...
| CLM-7 | Settings said *"Permanently delete your account and organization data. This can't be undone."* over a code path that deletes nothing, and the Privacy Policy promised post-closure deletion "within a reasonable period" with no purge job behind it. | #398 | **Done (#398, ADR 0013 Amendment 1)**: every claim was verified against the endpoint before a word was written. `AuthEndpoints.DeleteAccount` cancels any live Stripe subscription (aborting the whole request with a 502/503 if it cannot, so the billing half of the old copy was the one true clause), writes a `user.account_deleted` audit row with the identity still intact, overwrites `User.Email` with `deleted+{id}@deleted.invalid` and `FullName` with `"Deleted account"`, and stamps `DeletedAt` on the user and the organization. That is the whole of it. The vendors and their contact email/phone, the documents and their original file names, **every uploaded blob in Azure**, the reminders and reminder logs, the `Subscription` row and the entire audit trail all survive — hidden behind the soft-delete query filters, not purged (ADR 0013 § Consequences, which calls the boundary deliberate). ADR 0013 also lists *"reversibility by support (an accidental self-deletion can be undone by clearing `DeletedAt`)"* among its benefits, so *"can't be undone"* asserted the opposite of a designed-in property. Two more surfaces carried the same family and were corrected with it: the per-document and per-vendor removal dialogs (both endpoints soft-delete; `DeleteDocument` retains the blob **on purpose** — its own comment says the document *"remains recoverable"*), and the Settings data-export card, which offered *"a JSON copy of your … documents"* for an export that serializes document ROWS and no files at all. **The fix is option (a) of the ticket, not option (b)**: make every deletion claim true and route the retention question to counsel. Building the post-closure purge would reverse a standing ADR, need a retention schedule nobody has set, and need the very sign-off this row asks for — while the harm being reported is the DECEPTION, which corrected copy removes completely and immediately. Same posture as CLM-3/ADR 0047: ship the truthful wording now, mark it provisional. **No behaviour changed, and no retention period is stated anywhere** — stating a number no job enforces would recreate this exact defect in a new sentence, which is why the policy now says plainly that no fixed disposal period is set and that records stay until deletion is requested. **Attorney to confirm or refine sentences (a)–(d)** in the §0 register — each is a one-file edit, and none reaches a verdict — **and to rule on the open decision plus (i)–(iii)** carried there (a new item is added in BOTH places or in neither). The open decision is the one #398 deliberately did not answer in code: **build a post-closure hard purge and publish a retention schedule, or is retention-with-deletion-on-request the posture?** If it is the purge, the follow-on questions are what it removes (blobs and vendor contact PII at minimum), after how long, and what the audit trail and billing records are allowed to keep. If it is retention-on-request, then (iii) is the sharp end: the people whose PII we hold are largely **not** our customers — a vendor named on a certificate, or the account-less uploader who was sent a portal link — and after their customer closes the account there is nobody left who both knows we hold it and has an interest in asking. |
...
*Adjacent bundle: tickets #402–#405 and #398 (Part C) all touch legal/privacy copy
and can go in the same attorney engagement. Rule-engine flags stay OFF and template
