...
### Neutral
- The `portal-token` and `portal-ip` named policies are gone. Any future code that calls `.RequireRateLimiting("portal-token")` will fail at startup with an "unknown policy" error — that's the desired failure mode (loud, immediate) rather than silent drop.
