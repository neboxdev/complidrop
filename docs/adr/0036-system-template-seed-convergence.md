...
- `RegradedThroughRevision` advances to the re-graded revision **only when the fan-out reports FULL success.** `ReevaluateForTemplateForSystemAsync` now returns a `RegradeResult(Targeted, Regraded, FailedPages)`; the seed advances the watermark iff `FailedPages == 0`. A skipped page leaves the watermark behind, so the next boot re-fires the re-grade for that template until every page lands. The advance is written with `ExecuteUpdate` (not the tracked entity) because the shared fan-out clears the context's `ChangeTracker` per page, which would otherwise silently drop a tracked-property write. The deploy log now reports attempted-vs-succeeded (`re-graded {Regraded}/{Targeted} … {FailedPages} page(s) failed or unconfirmed`) so a partial re-grade is visible. **Amended by [ADR 0030 Amendment 5](0030-compliance-verdict-combined-unit-of-work.md#amendment-5-2026-08-03--the-batched-fan-out-verifies-the-page-it-just-committed) (#470):** `FailedPages` now counts a page the post-commit verification could not CONFIRM — whether it THREW or spent its bound on a document whose inputs kept moving — so it is no longer a strict attempted-vs-succeeded pair. Those documents stay inside `{Regraded}` (they were re-graded; what failed is the confirmation), which makes `Regraded == Targeted` beside a non-zero `FailedPages` a real line rather than a contradiction. The watermark rule is unchanged and covers the new arms for the same reason it covers the old one: a page whose verdicts are committed but UNCONFIRMED is exactly a page the next boot should re-fire. Both endings count, and that is load-bearing here specifically — the give-up arm originally returned normally without incrementing, so `AllSucceeded` stayed true and this watermark advanced over a document holding a verdict graded from inputs that had moved under it, which is precisely the false-verdict-survives-forever hole Amendment 2 exists to close.
