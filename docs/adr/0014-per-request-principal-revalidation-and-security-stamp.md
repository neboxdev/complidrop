...
- **Grandfathering leaves a bounded transition window.** A token minted *before* this deploy has no `stamp` claim and is therefore not rejected on the stamp check — so for already-issued tokens a password reset/change does **not** evict them until they age out (≤15 min for a session, **≤30 days for a refresh token**). That means defect #1 (a stolen refresh token surviving a takeover-recovery reset) stays open for pre-existing refresh tokens for up to the refresh TTL after deploy; it is fully closed only for tokens minted afterward. The soft transition was chosen over a hard cutover deliberately — a hard cutover would force a fleet-wide re-login on deploy — and the window self-heals as every token re-stamps on its next login/refresh. If the residual window is ever deemed unacceptable, a deploy-dated hard cutover (reject session tokens issued before date X) can be layered on later.
