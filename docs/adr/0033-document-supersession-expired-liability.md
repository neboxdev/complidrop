...
### Negative
- Each Expired count/filter gains a correlated `EXISTS` subquery. At SMB scale (a vendor has a handful of docs) the existing `(OrganizationId, VendorId)` index serves it adequately — the `EXISTS` narrows to one vendor's small doc set; no dedicated index was added. If a tenant ever grew large enough to matter, a `(VendorId, DocumentType, CreatedAt)` partial index would serve it directly.
