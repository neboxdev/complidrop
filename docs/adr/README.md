...
- [0030](0030-compliance-verdict-combined-unit-of-work.md) — Compliance verdict commits in the same transaction as its inputs (combined unit of work) (Amendment 1: one transaction is not enough for the two PARTIAL writers — `UpdateFields` / `UpdateDocument` run under `REPEATABLE READ` and re-run against a fresh read on a conflicting concurrent commit, bounded, 409 when exhausted; no `xmin` token, no schema change) (Amendment 2: the extraction worker grades a change-tracker-derived prediction of the row its own commit will LEAVE — `Services/DocumentGradingBasis` — instead of the minutes-old snapshot; read-only, so it still writes only the columns it extracted; the re-read→commit window stays open and is recorded) (Amendment 3: the SINGLE-document pure re-grade — `ComplianceEndpoints.RunCheck`, "Check again" — takes Amendment 1's `REPEATABLE READ` + bounded re-run, so a field edit landing in its window forces a recompute instead of leaving an affirmative verdict over lowered inputs; exhaustion answers 409 and leaves the previous verdict ALONE rather than degrading it; the guard is at the call site and pinned there, and the BATCHED fan-outs deliberately keep the window) (Amendment 4: a `ComplianceCheck` DELETE that matched nothing is a SUCCESS — a competing re-grade deleting the rows a writer staged for removal used to throw `DbUpdateConcurrencyException` out of the extraction persist, i.e. a re-paid OCR + LLM run on every zombie reclaim; `Data/ComplianceCheckDeleteConcurrencyInterceptor` tolerates exactly that, scoped to deletes of that one entity, and changes NOTHING about the unit of work — a set-based `ExecuteDeleteAsync` clear would have committed outside it)
...
- [0042](0042-distrusted-extraction-per-field-gate-and-coverage-exclusion.md) — A distrusted extraction (per-field confidence gate) routes to review and does not roll up to "Covered" (dovetails with 0040/0041; Amendment 1 records why its document-level carve-out does not generalize to 0048; Amendment 2 extends the coverage exclusion to a terminally `Failed` extraction NOBODY CONFIRMED — where a re-armed distrusted document lands — and records why its human exit was `IsManuallyVerified` rather than the status; Amendment 3 moves the distrust signal to its own column per 0052, closing that flag's stickiness and reversing the in-flight carve-out for a document that was already distrusted)
...
- [0047](0047-exports-carry-a-non-advice-disclaimer.md) — Every generated export (audit PDF, vendor package, CSV) carries one shared non-advice disclaimer, on by default rather than flag-staged (CLM-3); the PDFs render it in `page.Footer()` so it lands on every page
- [0048](0048-never-graded-document-asserts-no-affirmative-verdict.md) — A never-graded document (zero `ComplianceCheck` rows) asserts no affirmative verdict — read-only demotion to Pending on every surface, document-level included (follows 0042's coverage exclusion, declines its document-level carve-out)
- [0049](0049-clipped-extraction-field-is-disclosed-not-silent.md) — A clipped extraction field is disclosed at read time (a `fieldValueTruncated` flag derived from the jsonb copy vs the clamped row), so a manual save is never an unannounced narrowing of the canonical value; the ADR 0045 §4 truncation itself is unchanged
- [0050](0050-reextract-refuses-a-live-extraction-claim.md) — A re-extract never re-arms a document whose extraction claim is still live: one conditional `ExecuteUpdateAsync` on the WORKER's own staleness rule (`ExtractionWorker.ZombieClaimTimeout`, now the single source `ClaimSql` is built from), 409 when refused; the `(DocumentId, FieldName)` unique index is deferred, not forgotten (Amendment 1 corrects the "no frontend change" consequence — the 409 is only reachable from a client that believes the read finished, so the detail page refetches on that code)
- [0051](0051-untrusted-extraction-input-is-not-instruction.md) — Untrusted extraction input is data, never instruction: the document-type hint carries a member of the canonical vocabulary or is not emitted at all (a point-of-use guard, because ADR 0045's legacy rows are deliberately un-laundered), and the shared system prompt states that the vendor-authored OCR block is untrusted content whose instructions are never followed; the two providers' prompt builders collapse into one
- [0052](0052-extraction-trust-is-its-own-column.md) — Extraction TRUST gets its own column (`Document.ExtractionTrust`) so a pipeline re-arm can no longer destroy it: written by the four events that establish or undermine a document's basis and by nothing in the queue path, read only by the vendor rollup, seeded for existing rows by an additive backfill that reproduces the pre-#459 exclusion predicate exactly (amends 0042)
