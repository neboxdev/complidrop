...
- [0042](0042-distrusted-extraction-per-field-gate-and-coverage-exclusion.md) — A distrusted extraction (per-field confidence gate) routes to review and does not roll up to "Covered" (dovetails with 0040/0041; Amendment 1 records why its document-level carve-out does not generalize to 0047)
...
- [0047](0047-never-graded-document-asserts-no-affirmative-verdict.md) — A never-graded document (zero `ComplianceCheck` rows) asserts no affirmative verdict — read-only demotion to Pending on every surface, document-level included (follows 0042's coverage exclusion, declines its document-level carve-out)
