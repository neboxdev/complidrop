    ...
    IReadOnlyDictionary<string, string?>? configOverrides = null) : WebApplicationFactory<Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureTestServices(services =>
        {
            ...
            // Lets a test fail ONE reader command on SystemDbContext before it reaches Postgres (#460
            // review round 2) — the only way to pin that the worker's grading-basis read sits inside the
            // degrade-to-Pending try rather than becoming a new way for the persist to throw. Same
            // callback arming and containment as the hook above.
