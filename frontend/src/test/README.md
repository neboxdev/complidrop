...
| `security.ts`          | `assertNotInDom(value, root?)` — assert a sensitive value is NOT rendered into the DOM (scans both `textContent` and `innerHTML`). **DOM only** — its docstring scopes it out of non-DOM channels, so it never speaks for what leaves over the wire (see `lib/analytics.test.ts` for the telemetry half). |
| `counsel-brief.ts`     | `counselRegisterRow(item)` + `flattenCopy(s)` — read one §0 row of `docs/rule-engine/G1-COUNSEL-BRIEF.md` and the copy it asks counsel to bless. Import it DIRECTLY (not via `@/test`) — it is fs-only, so a non-rendering assertion shouldn't pull in RTL/MSW. Consumers: `marketing-claims.test.ts` (CLM-4), `app/portal/[token]/page.test.tsx` (CLM-5). Adding a third means adding it to `frontend-ci.yml`'s trigger comment too — pinned by `counsel-brief.test.ts`. |
...
- `vi.useFakeTimers({ shouldAdvanceTime: true })` is REQUIRED for RTL's `waitFor` to work — without it the `waitFor` polling loop blocks on the fake-timer queue.
  - **Exception: a test that polls for nothing inside the fake window must leave it OFF.** "Required" means required for `waitFor`-based polling, not universally. When the point of the fake clock is to hold a precise interleaving — no `waitFor` / `findBy*` between `useFakeTimers()` and `useRealTimers()`, every advance inside an explicit `act` scope — auto-advance lets real wall-clock time under CI load fire the timer on its own, outside that scope, and the constructed window re-opens as a flake. The one such site is the "keeps characters typed in the same drain as the debounce write" test in `documents/page.test.tsx` (#450); its bare `vi.useFakeTimers()` is load-bearing and carries a comment saying so. Do not normalize it.
...
- **Default (setup-file mock + harness options).** `vitest.setup.ts` mocks `next/navigation` once against the mutable `navState`. Most tests use this — they drive routing through `renderWithProviders({ router, params, searchParams, pathname })` and assert on the returned spies (or `navState.router.push.mock.calls`). `notFound()` and `redirect()` throw a `NEXT_NOT_FOUND` / `NEXT_REDIRECT` sentinel so component code after them cannot silently keep running.
  ...
  **`window.history.pushState` / `replaceState` are bridged too, and they SPLIT (#370).** Next's App Router integrates the native History API: those calls update the URL and sync `usePathname`/`useSearchParams` with no route navigation and no RSC fetch (the documented path for list filter/sort state — the documents page uses it). But the two halves do not land together:
  ...
  - `useSearchParams()` / `usePathname()` update **a commit later** — Next's patch (`app-router.js`, `applyUrlFromHistoryPushReplace`) dispatches ACTION_RESTORE, and the AppRouter's `setState` is only reached from the action queue's async reducer microtask. (The `startTransition` around that dispatch does not make the update a transition; the microtask is what defers the snapshot. See the `setNavigationCommitDelay` note below.) Skipping the RSC fetch makes this window narrower than `router.replace`'s, not absent.

  This was previously documented — and mocked — as fully synchronous, with a "don't collapse it" note attached. That was wrong, and it cost a whole review pass: a page composed its filter writes on the deferred hook, and the synchronous mock meant its own regression tests passed anyway. **When you need "has the URL changed", assert `window.location.search`; use `navState.searchParams` only when the router snapshot is itself the subject, and `await` it.**
  ...
  **How a page resolves this** (the documents page is the reference implementation): take `useSearchParams()` as the base — it is correct for every change the page did not make — and overlay the page's OWN writes on top until the router echoes them back. Hold those pending writes as a **queue**, not a single value: two writes can be in flight at once, and when the first lands the router snapshot equals neither the pre-write base nor what the user last picked, so a single-value overlay reads it as an external navigation and discards the second write. Anything the router reports that is not in the queue *is* an external navigation and supersedes the overlay.
  ...
  `setNavigationCommitDelay(ms)` staggers commit latency within a test, for BOTH mechanisms. Real `router` commits wait on their own RSC fetch, so two dispatched together land at different times — with a single shared deadline that interleaving is untestable. A History-API commit waits on the router's async reducer instead, which React can schedule late or interrupt; same knob, since what a test cares about is how long the component keeps reading the old value. Reset to 0 between tests.

  **Under fake timers the default 0 is a trap (#450).** `applyHistoryUrl` arms its deferred commit AT the instant `replaceState` runs, so a single `advanceTimersByTime` that reaches a History write also drains that write's echo — and the harness's echo re-renders through `useSyncExternalStore`, i.e. at **SyncLane**, so it joins whatever else that advance scheduled instead of landing after it. Production cannot order it that way: the echo is a plain **DefaultLane** update on the AppRouter (the `startTransition` around the ACTION_RESTORE dispatch never reaches the `setState`), so it lands *with* the page's own DefaultLane updates or later, never at SyncLane ahead of them. So when a test advances a fake clock across a History write, push the delay past the advance (`setNavigationCommitDelay(SEARCH_DEBOUNCE_MS * 10)`) to keep the echo out of the window. Worked example: the "keeps characters typed in the same drain as the debounce write" test in `documents/page.test.tsx`. **The `setNavigationCommitDelay` JSDoc in [navigation.ts](./navigation.ts) is the authority for the full derivation** (which Next source lines, and why the same-commit ordering production actually takes is not expressible here) — keep the two in step rather than restating it.
